...
class Controller:
    ...
    Construction connects an admin queue, enables and identifies the remote
    controller, then creates one direct I/O queue. Use :meth:`connect` for a
    descriptive constructor or instantiate the class directly. Controllers
    are context managers and should be closed after every registered buffer.

    Args:
        host: Target hostname or IP address.
        subsystem_nqn: NQN of the target subsystem to connect to.
        port: NVMe/RDMA service port, normally
            :data:`nvmeof.protocol.NVME_RDMA_PORT`.
        ...
        queue_depth: Requested I/O queue depth from 2 through 256. The target
            may negotiate a smaller depth.
        keep_alive_ms: Keep-alive timeout. Only zero is currently supported.
        timeout: Command timeout in seconds.
        source: Optional initiator IP address used to select and bind the
            local RDMA device.
    """
    ...
    def identify_namespace(self, nsid: int) -> "Namespace":
        ...
        Namespaces using separate metadata are rejected because this
        initiator currently transfers data blocks only.
        """
        nsid = int(nsid)
    ...
    def allocate(self, length: int) -> HostBuffer:
        ...
        ``length`` is the positive allocation size in bytes. The returned
        buffer is owned by the controller's I/O protection
        domain and must be closed before the controller.
        """
    ...
    def register(self, tensor):
        ...
        The returned MR retains the allocation and must be closed before the
        controller. Use :meth:`register_gpu` for CUDA tensors.
        """
    ...
    def register_gpu(self, tensor):
        ...
        The returned :class:`ibverbs.cuda.GpuMR` retains ``tensor`` and must
        be closed before the controller.
        """
...
class Namespace:
    ...
    :attr:`info` contains the full identified geometry; :attr:`nsid` and
    :attr:`lba_size` provide the fields most I/O callers need.
    """
    ...
    def read(self, buffer, slba: int, blocks=None, *, buffer_offset: int = 0) -> None:
        ...
        ``blocks`` defaults to the whole remaining buffer and must describe a
        whole number of LBAs. Transfers exceeding MDTS are split into serial
        commands. A GPU read flushes completed GPUDirect writes before return.

        Args:
            buffer: Registered host MR or :class:`ibverbs.cuda.GpuMR`.
            slba: Starting logical block address in the namespace.
            blocks: Logical block count, or ``None`` to use the remaining
                registered buffer.
            buffer_offset: Starting byte offset in ``buffer``.
        """
    ...
    def write(self, buffer, slba: int, blocks=None, *, buffer_offset: int = 0) -> None:
        ...
        ``blocks`` defaults to the whole remaining buffer and must describe a
        whole number of LBAs. Transfers exceeding MDTS are split into serial
        commands. CUDA work is synchronized before a GPU write is submitted.
