...
def test_atomic_fetch_add(pair):
    ...
    a.qp.post_send(
        ib.SendWR(
            wr_id=40,
            sg_list=[result.sge(8)],
            opcode=ib.WROpcode.ATOMIC_FETCH_AND_ADD,
            send_flags=ib.SendFlags.SIGNALED,
...
def test_atomic_compare_swap(pair):
    ...
    a.qp.post_send(
        ib.SendWR(
            wr_id=50,
            sg_list=[result.sge(8)],
            opcode=ib.WROpcode.ATOMIC_CMP_AND_SWP,
            send_flags=ib.SendFlags.SIGNALED,
            remote_addr=target.addr,
            rkey=target.rkey,
            compare_add=0x1122334455667788,
            swap=0xAABBCCDD,
        )
    )
    wc = a.poll_one()
